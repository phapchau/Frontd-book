<template>
    <div class="page row">
        ...
        <div class="mt-3 col-md-6">
            ...
            <p v-else>Không có theo doi nao.</p>
            <div class="mt-3 row justify-content-around align-items-center">
                ...
                <button class="btn btn-sm btn-success" @click="goToAddTheodoi">
...
<script>
import TheodoiCard from "../components/TheodoiCard.vue";
import InputSearch from "@/components/InputSearch.vue";
import TheodoiList from "@/components/TheodoiList.vue";
...
export default {
    ...
    methods: {
        async retrieveContacts() {
            try {
                this.contacts = await TheodoiService.getAll();
            } catch (error) {
                console.log(error);
            }
        },
        ...
        async removeAllContacts() {
            ...
            if (confirm("Bạn muốn xóa tất cả Liên hệ?")) {
                try {
                    await TheodoiService.deleteAll();
                    this.refreshList();
                } catch (error) {
                    console.log(error);
                }
        ...
        goToAddNXB() {
            this.$router.push({ name: "theodoi.add" });
        },
        goToLogIn() {
